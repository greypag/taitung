		...
		if ( $this->must_render( $languages ) ) {
			...
			if ( is_array( $hreflang_items ) ) {
				foreach ( $hreflang_items as $hreflang_code => $hreflang_url ) {
					if ((strpos($hreflang_url, '/zh-hans/') == false) && (strpos($hreflang_url, '/en/') == false) && (strpos($hreflang_url, '/ja/') == false) && (strpos($hreflang_url, '/ko/') == false)) {
						$hreflang .= '<link rel="alternate" hreflang="' . esc_attr( "x-default" ) . '" href="' . esc_url( $hreflang_url ) . '" />' . PHP_EOL;
					}else {
